...
train = daily_returns[daily_returns.index < split_date]
test = daily_returns[daily_returns.index >= split_date]

# Fit scaler and PCA on train only
scaler_oos = StandardScaler()
train_scaled = pd.DataFrame(
    scaler_oos.fit_transform(train[commodity_columns]),
    columns=commodity_columns,
    index=train.index
)
test_scaled = pd.DataFrame(
    scaler_oos.transform(test[commodity_columns]),
    columns=commodity_columns,
    index=test.index
)

# Scale EURUSD separately
eurusd_scaler = StandardScaler()
train_eurusd = eurusd_scaler.fit_transform(train[["EURUSD"]]).flatten()
test_eurusd = eurusd_scaler.transform(test[["EURUSD"]]).flatten()

# Fit PCA on train only
pca_oos = PCA(n_components=n_components)
X_train_pca = pca_oos.fit_transform(train_scaled)
X_test_pca = pca_oos.transform(test_scaled)

# Fit OLS on train only
X_train_oos = sm.add_constant(X_train_pca)
X_test_oos = sm.add_constant(X_test_pca)
model_oos = sm.OLS(train_eurusd, X_train_oos).fit()

# Predict on test set — frozen model
test_predictions = model_oos.predict(X_test_oos)

# ── PRINT RESULTS ─────────────────────────────────────────────────────────────
rmse_oos = np.sqrt(np.mean((test_eurusd - test_predictions) ** 2))
corr_oos, p_oos = stats.pearsonr(test_eurusd, test_predictions)
r2_oos = corr_oos ** 2

rmse_is = np.sqrt(np.mean((train_eurusd - model_oos.predict(X_train_oos)) ** 2))
corr_is, _ = stats.pearsonr(train_eurusd, model_oos.predict(X_train_oos))
r2_is = corr_is ** 2

print("\n" + "=" * 50)
print("OUT-OF-SAMPLE VALIDATION RESULTS")
print("=" * 50)
print(f"{'Metric':<20} {'In-Sample':>12} {'Out-of-Sample':>15}")
print("-" * 50)
print(f"{'R-squared':<20} {r2_is:>12.4f} {r2_oos:>15.4f}")
print(f"{'Correlation':<20} {corr_is:>12.4f} {corr_oos:>15.4f}")
print(f"{'RMSE':<20} {rmse_is:>12.4f} {rmse_oos:>15.4f}")
print(f"{'P-value':<20} {'—':>12} {p_oos:>15.6f}")
print("=" * 50)
print(f"Train period: 2016-01-01 to {split_date}")
print(f"Test period:  {split_date} to 2026-06-01")
